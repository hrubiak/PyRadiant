...
class Poller(QtCore.QObject):
    ...
    def __init__(self, ol: OL16A, interval_ms: int = 250):
        super().__init__()
        self.ol = ol
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.lblI = QtWidgets.QLabel("0.0000 A")
        self.lblV = QtWidgets.QLabel("0.0000 V")
    ...
    def _update_readings(self, data: dict):
        # data: {'amps','volts','target','state'}
        amps = data.get("amps", 0.0)
        volts = data.get("volts", 0.0)
        t = data.get("target", {})
        st = data.get("state", {})

        self.lblI.setText(f"{amps:.4f} A")
        self.lblV.setText(f"{volts:.4f} V")
        if t:
            self.lblTarget.setText(f"{t.get('value', 0):.4f} {t.get('unit','')}")
        flags = []
        flags += t.get("flags", [])
        flags += st.get("flags", [])
        self.lblFlags.setText(", ".join(sorted(set(flags))) if flags else "—")

        on = st.get("on", False)
        self.lblLamp.setText("ON" if on else "OFF")
        self.lblLamp.setStyleSheet("font-weight:bold; color:#009900" if on else "font-weight:bold; color:#aa0000")

    def _log(self, msg: str, is_error: bool = False):
        color = "#aa0000" if is_error else "#444"
        safe = html.escape(msg)   # <-- escape HTML properly
