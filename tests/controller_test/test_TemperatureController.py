...
from t_rax.widget.TemperatureWidget import TemperatureWidget
...
class TestTemperatureController(QtTest):
    ...
    def test_downstream_calibration_controls(self):
        ...
        self.controller.load_ds_standard_file(filename)
        self.assertEqual(self.model.ds_temperature_model.calibration_parameter.standard_file_name, filename)
    ...
    def test_upstream_calibration_controls(self):
        ...
        filename = os.path.join(temperature_fitting_path, '15A_lamp.txt')
        self.controller.load_us_standard_file(filename)
        self.assertEqual(self.model.us_temperature_model.calibration_parameter.standard_file_name, filename)
        self.assertEqual(str(self.widget.us_standard_filename_lbl.text()), os.path.basename(filename))
    ...
    def test_loading_settings(self):
        ...
        self.assertEqual(self.widget.ds_standard_filename_lbl.text(),
                         '15A_lamp.txt')
        self.assertEqual(self.widget.us_standard_filename_lbl.text(),
                         '15A_lamp.txt')
    ...
    def load_single_frame_file_and_calibration(self):
        ...
        self.model.load_ds_calibration_image(os.path.join(
            temperature_fitting_path,
            'ds_calibration.spe'))
        # load correct standard files:
        self.model.load_us_standard_spectrum(os.path.join(
            temperature_fitting_path,
            '15A_lamp.txt'
        ))
        self.model.load_ds_standard_spectrum(os.path.join(
            temperature_fitting_path,
            '15A_lamp.txt'
        ))
